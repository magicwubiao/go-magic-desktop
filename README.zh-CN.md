...
- **外部链接处理**: 自动在系统浏览器中打开非本地链接
- **后端重启**: 无需关闭应用即可从 UI 重启后端
- **应用信息查询**: 运行时获取版本、git 提交和构建元数据
- **日志系统**: 结构化日志便于问题排查
- **安全策略**: CSP 防护、权限控制
...
2. **启动应用** — 打开应用（窗口标题为 **“Go Magic”**）。桌面端会自动完成以下工作：
   ...
   - 启动后端并最多等待 60 秒进行健康检查；
   - 在窗口内加载 Go Magic 的 Web 界面。
...
- **build**: 多平台桌面应用构建（Windows/macOS-x64/macOS-arm64/Linux）
...
构建时，`build.rs` 运行 `git describe --tags` 将真实版本（以及 git 提交、
构建时间、构建配置）嵌入二进制文件。CI 标签推送时，标签版本也会注入
`tauri.conf.json`，使安装包携带正确版本。
